...
def model_run(args, ae_model_path, ae_model_type, depth_e1, drop_ratio, learning_rate):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            torch_state, np_state, py_state = handle_rng_states('save')
            val(args, model, val_loader)
            handle_rng_states('load', torch_state, np_state, py_state)
            aucs = test(args, epoch, model, test_dataset, test_loader, test_smiles, ae_model_type, depth_e1, drop_ratio, learning_rate)
...
def handle_rng_states(mode, torch_state=None, np_state=None, py_state=None):
    if mode == 'save':
