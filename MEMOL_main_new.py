...
# from model_MoE_Cross_hyper_tune_ablation1 import MCLDTI
from model_MoE_Cross_hyper_tune_ablation2 import MCLDTI
# from model_MoE_Cross import MCLDTI
...
def model_run(args, ae_model_path, ae_model_type, depth_e1, drop_ratio, learning_rate):
    ...
    result_file = resul_name + "/" + run_time + \
                  "_learnable" + \
                  "_Testing" + \
                  f"_depth_e1_{depth_e1}" + \
                  f"_drop_ratio_{drop_ratio}" + \
                  f"_lr_{learning_rate}" + ".txt"
    ...
    model = MCLDTI(depth_e1=depth_e1,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # model_file = model_path + run_time + "_" + str(epoch) + ".model"                                              # model save delete 25.02.04
        # torch.save(model, model_file)
        print("Epoch: ", epoch, "     Train avg_loss: ", sum(total_loss) / len(train_loader))

        with torch.no_grad():
            # aucs = test(args, epoch, model_file, test_dataset, test_loader, test_smiles, ae_model_type, depth_e1,      # model save delete 25.02.04
...
def test(args, epoch, file_model, test_dataset, test_loader, smiles, ae_model_type, depth_e1, drop_ratio,
    ...
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_self_sparse_cross_default_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_self_default_cross_sparse_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_self_sparse_cross_sparse_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"
    ...
    ############# ablation 2 ##############
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_ablation2_img_gr_to_fp_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"  # state 0, original
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_ablation2_img_fp_to_gr_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"  # state 1, img + fp -> graph
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_ablation2_gr_fp_to_img_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"  # state 2, gr + fp -> im
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_ablation2_img_gr_fp_concat_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"  # state 3, img_gr_fp_concat
    # directory = f"result/{args.dataset_name}/{args.dataset_name}_ablation2_img_gr_fp_normal_concat_{ae_model_type}_depth{depth_e1}_drop{drop_ratio}_lr{learning_rate}"  # state 3, img_gr_fp_normal_concat
...
def hyperparameter_tuning(args, ae_model_info):
    # depth_e1_values = [4]
    # drop_ratio_values = [0.0]
    ...
    ##### carcino #####
    # depth_e1_values = [4]
    # drop_ratio_values = [0.3]
    # learning_rate_values = [1e-3]

    ##### DILIst #####
    # depth_e1_values = [2]
    # drop_ratio_values = [0.1]
    # learning_rate_values = [1e-3]

    best_results = []

    for ae_model_path, ae_model_type in ae_model_info:
        for depth_e1 in depth_e1_values:
            for drop_ratio in drop_ratio_values:
                for learning_rate in learning_rate_values:
                    ...
                    # result_file_path = f'result/{args.dataset_name}_hyperparameter_tuning_self_sparse_cross_default.txt'
                    # result_file_path = f'result/{args.dataset_name}_hyperparameter_tuning_self_default_cross_sparse.txt'
                    # result_file_path = f'result/{args.dataset_name}_hyperparameter_tuning_self_sparse_cross_sparse.txt'

                    ###### ablation 2 ######
                    # result_file_path = f'result/{args.dataset_name}_ablation2_sparseMoE_img_gr_to_fp.txt'
                    # result_file_path = f'result/{args.dataset_name}_ablation2_sparseMoE_img_fp_to_gr.txt'
                    # result_file_path = f'result/{args.dataset_name}_ablation2_sparseMoE_gr_fp_to_img.txt'
                    # result_file_path = f'result/{args.dataset_name}_ablation2_sparseMoE_img_gr_fp_concat.txt'
                    # result_file_path = f'result/{args.dataset_name}_ablation2_sparseMoE_img_gr_fp_normal_concat.txt'
                    ###### ablation 2 ######
...
def main():
    ...
    # ae_model_info = [
    #    ('data/ClinTox/autoencoder_layer2_ep100.pth', 'ep100'),
    #    ('data/ClinTox/autoencoder_layer2_ep60.pth', 'ep60'),
    #    ('data/ClinTox/autoencoder_layer2_ep80.pth', 'layer2'),
    # ]

    # ae_model_info = [
    #    ('data/DILIst/autoencoder_layer2_ep70.pth', 'ep70')
    #    ('data/DILIst/autoencoder_layer2_ep110.pth', 'layer2'),
    #    ('data/DILIst/autoencoder_layer2_ep160.pth', 'layer2'),
    # ]
    ...
    hyperparameter_tuning(opt, ae_model_info)
...
def set_seed(seed):
    ...
    torch.cuda.manual_seed(seed)  # PyTorch GPU 연산 시드 고정
    torch.cuda.manual_seed_all(seed)  # Multi-GPU 연산 시드 고정
    torch.backends.cudnn.deterministic = True  # CUDNN 결정론적 연산 활성화
    torch.backends.cudnn.benchmark = False  # 결정론적 연산을 위한 비활성화
